...
edit: (props) => {
  ...
  const { Placeholder, PanelBody, SelectControl, ToggleControl, TextControl } = wp.components;
  const { InspectorControls, useBlockProps } = wp.blockEditor;

  const { attributes, setAttributes } = props;
  const blockProps = useBlockProps();

  // Lokaler State spiegelt Attribute
  ...
  return wp.element.createElement(
    ...
    wp.element.createElement(
      ...
      wp.element.createElement(
        PanelBody,
        { title: __('Darstellung', 'hcs'), initialOpen: true },
      ...
      wp.element.createElement(
        ...
        // Falls NumberControl nicht verfügbar ist, bleib bei TextControl
        wp.element.createElement(TextControl, {
          label: __('Verzögerung (ms)', 'hcs'),
          value: attributes.autoplayDelay,
          type: 'number',
          onChange: (v) => setAttributes({ autoplayDelay: parseInt(v, 10) || 5000 })
        }),
