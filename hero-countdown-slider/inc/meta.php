...
function hcs_slide_meta_cb($post){
	...
	<div class="hcs-grid">
		<div class="hcs-row">
			<label>Titel</label>
			<input type="text" name="hcs_title" class="widefat" value="<?php echo esc_attr($fields['title']); ?>">
			<div class="hcs-small">Nur nutzen, wenn kein Logo angezeigt wird.</div>
		</div>
		<div class="hcs-row">
			<label>Untertitel</label>
			<input type="text" name="hcs_subtitle" class="widefat" value="<?php echo esc_attr($fields['subtitle']); ?>">
		</div>
		...
		<div class="hcs-row">
			<label>Countdown bis (ISO, z. B. 2030-01-01T00:00:00)</label>
			<input type="text" name="hcs_countdown" class="widefat" value="<?php echo esc_attr($fields['countdown']); ?>">
		</div>
		<div class="hcs-row">
			<label>Gültig bis (ISO, z. B. 2030-01-01T12:00:00)</label>
			<input type="text" name="hcs_expires_at" class="widefat" value="<?php echo esc_attr($fields['expires_at']); ?>">
			<div class="hcs-small">Nach Ablauf wird die Slide automatisch ausgeblendet.</div>
		</div>

		<div class="hcs-row">
			<label>CTA Label</label>
			<input type="text" name="hcs_cta_label" class="widefat" value="<?php echo esc_attr($fields['cta_label']); ?>">
		</div>
		<div class="hcs-row">
			<label>CTA URL</label>
			<input type="url" name="hcs_cta_url" class="widefat" value="<?php echo esc_attr($fields['cta_url']); ?>">
		...
		(function($){
			...
			$('#hcs_pick_logo').on('click', function(e){
				...
				frame.on('select', function(){
					...
					$('#hcs_logo_preview').attr('src', att.sizes?.thumbnail?.url || att.url).show();
				});
				frame.open();
			});
		})(jQuery);
